...
{
    public partial class CarInformationFormOne : Form
    {
        public CarInformationFormOne(string _carIds, bool carinfo)
        {
            ///carinfo为false： 卡
            if (carinfo == false)
            {
        ...
        public static CarInformationFormOne cinfo = null;
        ...
        /// 存放有效图片
        /// </summary>
        List<string> listCarPicEffective;
        /// <summary>
        /// 存放过期图片
        /// </summary>
        ...
        private void CarInformationForm_Load(object sender, EventArgs e)
        {
            ...
            CommonalityEntity.isRestFXOne = false;
        ...
        private void AddCarInOutRecordMethod()
        {
            ...
            if (!CommonalityEntity.IsUpdatedri)
            {
                ...
                Strategy_DriSName = CheckProperties.dt.Rows[0]["CarInOutRecord_Remark"].ToString();
            ...
            if ((isfull && CommonalityEntity.Car_ISRegister) || (isbool && CommonalityEntity.Car_ISRegister))  //业务完成，并且只登记一次车辆，添加车辆通行总记录等信息；当本次的通行状态与前一次通行状态相同时，为异常进出厂，添加通行总记录
            {
                ...
                cir.CarInOutRecord_Driveway_ID = Convert.ToInt32(LinQBaseDao.Query("select DrivewayStrategy_Driveway_ID from DrivewayStrategy where DrivewayStrategy.DrivewayStrategy_ID=" + cir.CarInOutRecord_DrivewayStrategy_ID).Tables[0].Rows[0][0]);
                if (issort)
                {
        ...
        private void CarInformationForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                ...
                pictureBox2.Dispose();
                CommonalityEntity.isRestFXOne = false;
                CarInformationFormOne.cinfo = null;
        ...
        private void btn_ConfirmationRelease_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                CommonalityEntity.isRestFXOne = true;
        ...
        public void cardinfoRecord()
        {
            ...
            {
                CommonalityEntity.WriteTextLog("CarInformationForm.cardinfoRecord()");
            }
        }
        ...
        private bool opDoorPort(int address)
        {
            ...
            CommonalityEntity.contolone = contio;
        ...
        {
            if (LinQBaseDao.Query("select * from Blacklist where Blacklist_CarInfo_ID in(select Car_ID from Car where Car_Name ='" + txt_Carnumber.Text + "')").Tables[0].Rows.Count >= 1)
            {
                //黑名单次数减1
