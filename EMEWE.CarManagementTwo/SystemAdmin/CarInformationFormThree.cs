...
{
    public partial class CarInformationFormThree : Form
    {
        public CarInformationFormThree(string _carIds, bool carinfo)
        {
            ///carinfo为false： 卡
            if (carinfo == false)
            {
        ...
        public static CarInformationFormThree cinfo = null;
        ...
        private void CarInformationForm_Load(object sender, EventArgs e)
        {
            ...
            CommonalityEntity.isRestFXThree = false;
        ...
        private void AddCarInOutRecordMethod()
        {
            ...
            if (!CommonalityEntity.IsUpdatedri)
            {
                ...
                {
                    if (DrivewayStrategy_IDDT.Rows[i]["DrivewayStrategy_Driveway_ID"].ToString() == CommonalityEntity.Driveway_ID.ToString())
                    {
            ...
            else
            {
                ...
                for (int i = 0; i < DrivewayStrategy_IDDT.Rows.Count; i++)
                {
                    if (DrivewayStrategy_IDDT.Rows[i]["DrivewayStrategyRecord_Driveway_ID"].ToString() == CommonalityEntity.Driveway_ID.ToString())
                    {
            ...
            if ((isfull && CommonalityEntity.Car_ISRegister) || (isbool && CommonalityEntity.Car_ISRegister))  //业务完成，并且只登记一次车辆，添加车辆通行总记录等信息；当本次的通行状态与前一次通行状态相同时，为异常进出厂，添加通行总记录
            {
                ...
                {
                    if (DrivewayStrategy_dt.Rows[i]["DrivewayStrategy_Driveway_ID"].ToString() == CommonalityEntity.Driveway_ID.ToString())
                    {
        ...
        private void CarInformationForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                ...
                pictureBox2.Dispose();
                CommonalityEntity.isRestFXThree = false;
                CarInformationFormThree.cinfo = null;
        ...
        private void btn_ConfirmationRelease_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                CommonalityEntity.isRestFXThree = true;
        ...
        private bool opDoorPort(int address)
        {
            ...
            CommonalityEntity.contolthree = contio;
