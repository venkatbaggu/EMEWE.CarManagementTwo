...
{
    public partial class LoadSmallDiscern : Form
    {
        public LoadSmallDiscern()
        {
            InitializeComponent();
        }
        ...
        private void LoadSmallDiscern_Load(object sender, EventArgs e)
        {
            userContext();
            WeighStrategyName();
        ...
        private void userContext()
        {
            ...
            else
            {
                ...
                btnSave.Visible = ControlAttributes.BoolControl("btnSave", "LoadSmallDiscern", "Visible");
                btnSave.Enabled = ControlAttributes.BoolControl("btnSave", "LoadSmallDiscern", "Enabled");

                btnLoading.Visible = ControlAttributes.BoolControl("btnLoading", "LoadSmallDiscern", "Visible");
                btnLoading.Enabled = ControlAttributes.BoolControl("btnLoading", "LoadSmallDiscern", "Enabled");

                btnOutPass.Visible = ControlAttributes.BoolControl("btnOutPass", "LoadSmallDiscern", "Visible");
                btnOutPass.Enabled = ControlAttributes.BoolControl("btnOutPass", "LoadSmallDiscern", "Enabled");
            }
        }
        /// <summary>
        /// 地磅房名称绑定
            ...
            if (ds.Rows.Count > 0)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        if (type == CommonalityEntity.loadFirstWeight)
                        {
                            txtInWeight.Text = dt.Rows[i]["BusinessRecord_Weight"].ToString();
                        }
                        if (type == CommonalityEntity.loadSecondWeight)
                        {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (table.Rows.Count > 0)
                {
                    ...
                    if (txtCartype_Name.Text == "成品国废车辆" || txtCartype_Name.Text == "内部成品国废车辆")
                    {
                        ////////////////////////////////////////
                        #region 特殊车辆
                        ...
                        for (int i = 0; i < tableB.Rows.Count; i++)
                        {
                            ...
                            if (tableB.Rows[i]["BusinessRecord_Type"].ToString() == CommonalityEntity.loadFirstWeight)
                            {
                                weight1 = tableB.Rows[i]["BusinessRecord_Weight"].ToString();
                            }
                            if (tableB.Rows[i]["BusinessRecord_Type"].ToString() == CommonalityEntity.unloadSecondWeight)
                            {
                                unload2 = tableB.Rows[i]["BusinessRecord_Weight"].ToString();
                            }
                        ...
                        if (string.IsNullOrEmpty(unload2))
                        {
                            MessageBox.Show("该车辆没有：" + CommonalityEntity.unloadSecondWeight);
                            return;
                        }
                        //对地磅策略判断   该磅房信息
                        ...
                        {
                            //判断第一次地磅是否重复记录
                            DataTable ds = LinQBaseDao.Query("select * from BusinessRecord where BusinessRecord_Type='" + CommonalityEntity.loadFirstWeight + "' and BusinessRecord_CarInOutRecord_ID=" + carinoutrecord + " order by BusinessRecord_ID desc").Tables[0];
                            if (ds.Rows.Count > 0)
                            {
                                if (MessageBox.Show("是否修改" + CommonalityEntity.loadFirstWeight + "重量？", "提示", MessageBoxButtons.YesNo) == DialogResult.Yes)
                                {
                                    LinQBaseDao.Query("BusinessRecord set BusinessRecord_Weight=" + txtinweight + " where BusinessRecord_CarInOutRecord_ID=" + carinoutrecord + " and BusinessRecord_Type='" + CommonalityEntity.loadFirstWeight + "'");
                                }
                                else
                                {
                                    return;
                            ...
                            else
                            {
                                if (type == CommonalityEntity.unloadSecondWeight)
                                {
                                    ...
                                    save(carinoutrecord, carinfo_po, CommonalityEntity.loadFirstWeight, txtinweight);
                        ...
                        {
                            //判断第二次地磅是否重复记录
                            DataTable ds = LinQBaseDao.Query("select * from BusinessRecord where BusinessRecord_Type='" + CommonalityEntity.loadSecondWeight + "' and BusinessRecord_CarInOutRecord_ID=" + carinoutrecord + " order by BusinessRecord_ID desc").Tables[0];
                            if (ds.Rows.Count > 0)
                            {
                                if (MessageBox.Show("是否修改" + CommonalityEntity.loadSecondWeight + "重量？", "提示", MessageBoxButtons.YesNo) == DialogResult.Yes)
                                {
                                    LinQBaseDao.Query("BusinessRecord set BusinessRecord_Weight=" + txtoutweight + " where BusinessRecord_CarInOutRecord_ID=" + carinoutrecord + " and BusinessRecord_Type='" + CommonalityEntity.loadSecondWeight + "'");
                                }
                                else
                                {
                                    return;
                            ...
                            {
                                DataTable tab = LinQBaseDao.Query("select * from View_BusinessRecord_CarInfo where BusinessRecord_Type='" + CommonalityEntity.loadFirstWeight + "' and BusinessRecord_CarInOutRecord_ID=" + carinoutrecord + " order by BusinessRecord_ID desc").Tables[0];
                                if (tab.Rows.Count > 0)
                                {
                                    if (tab.Rows[0]["BusinessRecord_UnloadEmpower"].ToString() != "1")
                                    {
                                        MessageBox.Show("请先装货授权，在添加过磅重量!");
                                        txtOutWeight.Text = "";
                                        return;
                                    }
                                    if (tab.Rows[0]["BusinessRecord_Type"].ToString() == CommonalityEntity.loadFirstWeight)
                                    {
                                        string carinfo_po = table.Rows[0]["CarInfo_PO"].ToString();
                                        save(carinoutrecord, carinfo_po, CommonalityEntity.loadSecondWeight, txtoutweight);
                                ...
                                {
                                    MessageBox.Show(this, "没有" + CommonalityEntity.loadFirstWeight);
                                    return;
                                }
                            }
                        }
                    ...
                    else
                    {
                        ...
                        {
                            MessageBox.Show("该车辆没有：" + CommonalityEntity.upWeight);
                            return;
                        }
                        //对地磅策略判断   该磅房信息
                        if (Convert.ToInt32(cobWeighStrategyRecord.SelectedValue) < 1)
                        ...
                        if (txtinweight != "" && txtinweight != weight1)
                        {
                            ...
                            if (ds.Rows.Count > 0)
                            {
                                if (MessageBox.Show("是否修改" + CommonalityEntity.loadFirstWeight + "重量？", "提示", MessageBoxButtons.YesNo) == DialogResult.Yes)
                                {
                                    LinQBaseDao.Query(" update BusinessRecord set BusinessRecord_Weight=" + txtinweight + " where BusinessRecord_CarInOutRecord_ID=" + carinoutrecord + " and BusinessRecord_Type='" + CommonalityEntity.loadFirstWeight + "'");
                                }
                                else
                                {
                                    return;
                        ...
                        if (txtoutweight != "" && txtoutweight != weight2)
                        {
                            ...
                            {
                                DataTable tab = LinQBaseDao.Query("select * from BusinessRecord where BusinessRecord_Type='" + CommonalityEntity.loadFirstWeight + "' and BusinessRecord_CarInOutRecord_ID=" + carinoutrecord + " order by BusinessRecord_ID desc").Tables[0];
                                if (tab.Rows.Count > 0)
                                {
                                    if (tab.Rows[0]["BusinessRecord_Type"].ToString() == CommonalityEntity.loadFirstWeight)
                                    {
            ...
            catch
            {
                ...
                CommonalityEntity.WriteTextLog("装货点过磅信息 btnSave_Click()");
            }
        }
        ...
        private void strsql()
        {
            ...
            DataTable dt = LinQBaseDao.Query("select ICCard_ID from ICCard WHERE ICCard_Value='" + iccard_id + "'").Tables[0];
            //刷IC卡
            if (dt.Rows.Count > 0)
            {
            ...
            if (ds.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < tableB.Rows.Count; i++)
                {
                    if (tableB.Rows[i]["BusinessRecord_Type"].ToString() == CommonalityEntity.loadSecondWeight)
                    {
                        ...
                        {
                            MessageBox.Show("车辆已装货确认！");
                            return;
                        }
                        loadtype = CommonalityEntity.loadSecondWeight;
                        weight2 = Convert.ToDouble(tableB.Rows[i]["BusinessRecord_Weight"].ToString());
                        siteid = Convert.ToInt32(tableB.Rows[i]["BusinessRecord_ID"].ToString());

                    }
                    if (tableB.Rows[i]["BusinessRecord_Type"].ToString() == CommonalityEntity.loadFirstWeight)
                    {
                        type = CommonalityEntity.loadFirstWeight;
                        weight1 = Convert.ToDouble(tableB.Rows[i]["BusinessRecord_Weight"].ToString());
                    }
                }

                if (loadtype == CommonalityEntity.loadSecondWeight && type == "")
                {
                    MessageBox.Show("没有" + CommonalityEntity.loadFirstWeight);
                    return;
                }
                if (loadtype == CommonalityEntity.loadSecondWeight && type == CommonalityEntity.loadFirstWeight)
                {
                    Expression<Func<BusinessRecord, bool>> p = n => n.BusinessRecord_CarInOutRecord_ID == int.Parse(carinout) && n.BusinessRecord_Type == CommonalityEntity.loadSecondWeight;
                    ...
                    {
                        CommonalityEntity.WriteLogData("新增", "装货点:车辆" + ds.Rows[0]["CarInfo_Name"] + "装货确认", CommonalityEntity.USERNAME);//添加操作日志
        ...
        private void btnLoading_Click(object sender, EventArgs e)
        {
            ...
            if (txtCartype_Name.Text == "成品国废车辆" || txtCartype_Name.Text == "内部成品国废车辆")
            {
                ...
                if (ds.Rows.Count > 0)
                {
                    ...
                    string carinout = ds.Rows[0]["BusinessRecord_CarInOutRecord_ID"].ToString();
                    DataTable tableB = LinQBaseDao.Query("select * from BusinessRecord where BusinessRecord_CarInOutRecord_ID=" + carinout).Tables[0];
                    for (int i = 0; i < tableB.Rows.Count; i++)
                    {
                        if (tableB.Rows[i]["BusinessRecord_Type"].ToString() == CommonalityEntity.loadFirstWeight)
                        {
                            ...
                            if (Convert.ToBoolean(ds.Rows[0]["CarInOutRecord_Update"].ToString()))
                            {
                                ...
                                sql = "Select * from ManagementStrategyRecord where ManagementStrategyRecord_State='启动' and ManagementStrategyRecord_CarType_ID in(select CarType_ID from CarType where CarType_Name='" + ds.Rows[0]["CarType_Name"].ToString() + "') and ManagementStrategyRecord_ControlInfo_ID in (select ControlInfo_ID from ControlInfo where  ControlInfo_IDValue='450301'  )";
                            }
                            else
                            {
                                CommonalityEntity.IsUpdatedri = false;
                                sql = "Select * from ManagementStrategy where ManagementStrategy_State='启动' and ManagementStrategy_CarType_ID in(select CarType_ID from CarType where CarType_Name='" + ds.Rows[0]["CarType_Name"].ToString() + "') and ManagementStrategy_ControlInfo_ID in (select ControlInfo_ID from ControlInfo where  ControlInfo_IDValue='450301'  )";
                            }
                            checkPr.ExecutionMethod(sql);
                            // CheckMethod.GoLoadSmallMethod();
                            if (CheckMethod.AbnormalInformation != "")
                            {
                                MessageBox.Show("过磅重量超出限定值，授权失败！");
                                return;
                            }
                            Expression<Func<BusinessRecord, bool>> p = n => n.BusinessRecord_CarInOutRecord_ID == int.Parse(carinout) && n.BusinessRecord_Type == CommonalityEntity.loadFirstWeight;
                            Action<BusinessRecord> ap = s =>
                            {
                                s.BusinessRecord_UnloadEmpowerPerson = CommonalityEntity.USERNAME;
                                s.BusinessRecord_UnloadEmpowerTime = CommonalityEntity.GetServersTime();
                            ...
                            if (BusinessRecordDAL.Update(p, ap))
                            {
                                CommonalityEntity.WriteLogData("新增", "装货点:车辆" + ds.Rows[0]["CarInfo_Name"] + "授权", CommonalityEntity.USERNAME);//添加操作日志
            ...
            else
            {
                ...
                if (ds.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < tableB.Rows.Count; i++)
                    {
                        if (tableB.Rows[i]["BusinessRecord_Type"].ToString() == CommonalityEntity.loadFirstWeight)
                        {
                            load = tableB.Rows[i]["BusinessRecord_Type"].ToString();
                        }
                        if (tableB.Rows[i]["BusinessRecord_Type"].ToString() == CommonalityEntity.unloadSecondWeight)
                        {
                            unload = tableB.Rows[i]["BusinessRecord_Type"].ToString();
                        }
                    }
                    for (int i = 0; i < tableB.Rows.Count; i++)
                    {
                        if (tableB.Rows[i]["BusinessRecord_Type"].ToString() == CommonalityEntity.loadFirstWeight)
                        {
                            ...
                            {
                                CommonalityEntity.IsUpdatedri = true;
                                sql = "Select * from ManagementStrategyRecord where ManagementStrategyRecord_State='启动' and ManagementStrategyRecord_CarType_ID in(select CarType_ID from CarType where CarType_Name='" + ds.Rows[0]["CarType_Name"].ToString() + "') and ManagementStrategyRecord_ControlInfo_ID in (select ControlInfo_ID from ControlInfo where  ControlInfo_IDValue='450101'  )";
                            }
                            else
                            {
                                CommonalityEntity.IsUpdatedri = false;
                                sql = "Select * from ManagementStrategy where ManagementStrategy_State='启动' and ManagementStrategy_CarType_ID in(select CarType_ID from CarType where CarType_Name='" + ds.Rows[0]["CarType_Name"].ToString() + "') and ManagementStrategy_ControlInfo_ID in (select ControlInfo_ID from ControlInfo where  ControlInfo_IDValue='450101'  )";
                            }
                            checkPr.ExecutionMethod(sql);
                            if (CheckMethod.AbnormalInformation != "")
                            {
